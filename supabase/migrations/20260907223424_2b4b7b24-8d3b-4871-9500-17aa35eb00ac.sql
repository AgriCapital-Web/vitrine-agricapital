...
CREATE POLICY "Users can view their own profile" ON public.profiles FOR SELECT TO authenticated USING (auth.uid() = user_id OR public.is_admin());
DROP POLICY IF EXISTS "Users can insert their own profile" ON public.profiles;
CREATE POLICY "Users can insert their own profile" ON public.profiles FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id OR public.is_admin());
DROP POLICY IF EXISTS "Users can update their own profile" ON public.profiles;
CREATE POLICY "Users can update their own profile" ON public.profiles FOR UPDATE TO authenticated USING (auth.uid() = user_id OR public.is_admin());
...
CREATE TABLE IF NOT EXISTS public.news (
  ...
  title_fr text NOT NULL, title_en text, title_ar text, title_es text, title_de text, title_zh text,
  content_fr text NOT NULL, content_en text, content_ar text, content_es text, content_de text, content_zh text,
  excerpt_fr text, excerpt_en text, excerpt_ar text, excerpt_es text, excerpt_de text, excerpt_zh text,
...
CREATE POLICY "Public can read published news" ON public.news FOR SELECT TO anon, authenticated USING (is_published = true OR public.is_admin());
DROP POLICY IF EXISTS "Admin full access news" ON public.news;
CREATE POLICY "Admin full access news" ON public.news FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE TABLE IF NOT EXISTS public.site_content (
  ...
  content_fr text, content_en text, content_ar text, content_es text, content_de text, content_zh text,
...
CREATE POLICY "Public can read active site content" ON public.site_content FOR SELECT TO anon, authenticated USING (is_active = true OR public.is_admin());
...
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.site_content FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

CREATE TABLE IF NOT EXISTS public.site_menu (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  parent_id uuid REFERENCES public.site_menu(id) ON DELETE CASCADE,
  label_fr text NOT NULL, label_en text, label_ar text, label_es text, label_de text, label_zh text,
  url text,
...
CREATE POLICY "Public can read active menu" ON public.site_menu FOR SELECT TO anon, authenticated USING (is_active = true OR public.is_admin());
DROP POLICY IF EXISTS "Admin manage site menu" ON public.site_menu;
CREATE POLICY "Admin manage site menu" ON public.site_menu FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
DROP TRIGGER IF EXISTS set_updated_at ON public.site_menu;
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.site_menu FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE POLICY "Public can read active media" ON public.site_media FOR SELECT TO anon, authenticated USING ((is_active = true AND is_archived = false) OR public.is_admin());
DROP POLICY IF EXISTS "Admin manage site media" ON public.site_media;
CREATE POLICY "Admin manage site media" ON public.site_media FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY "Anyone can submit a testimonial" ON public.testimonials FOR INSERT TO anon, authenticated WITH CHECK (true);
...
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.testimonials FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
DROP VIEW IF EXISTS public.testimonials_public CASCADE;
CREATE VIEW public.testimonials_public WITH (security_invoker = off) AS
SELECT id, first_name, last_name, testimonial, photo_url, is_agricapital_subscriber, status, created_at
FROM public.testimonials WHERE approved = true AND status = 'approved';
...
CREATE POLICY "Users manage own push subscriptions" ON public.push_subscriptions FOR ALL TO authenticated USING (user_id = auth.uid() OR public.is_admin()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "Anyone can record a page visit" ON public.page_visits FOR INSERT TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "Admin can view page visits" ON public.page_visits;
CREATE POLICY "Admin can view page visits" ON public.page_visits FOR SELECT TO authenticated USING (public.is_admin());
...
CREATE POLICY "Public can read visitor counters" ON public.visitor_counters FOR SELECT TO anon, authenticated USING (id = 'public');
DROP POLICY IF EXISTS "Admin manage visitor counters" ON public.visitor_counters;
CREATE POLICY "Admin manage visitor counters" ON public.visitor_counters FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
INSERT INTO public.visitor_counters (id, total_visitors, weekly_visitors, week_started_at)
VALUES ('public', 0, 0, date_trunc('week', now())) ON CONFLICT (id) DO NOTHING;

CREATE OR REPLACE FUNCTION public.bump_visitor_counters()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE current_week timestamptz := date_trunc('week', now());
BEGIN
  ...
  SET total_visitors = public.visitor_counters.total_visitors + 1,
      weekly_visitors = CASE WHEN public.visitor_counters.week_started_at < current_week THEN 1 ELSE public.visitor_counters.weekly_visitors + 1 END,
      week_started_at = CASE WHEN public.visitor_counters.week_started_at < current_week THEN current_week ELSE public.visitor_counters.week_started_at END,
      updated_at = now();
  RETURN NEW;
END; $$;
...
CREATE TRIGGER trg_bump_visitor_counters AFTER INSERT ON public.page_visits FOR EACH ROW EXECUTE FUNCTION public.bump_visitor_counters();
...
CREATE POLICY "Admin can view ai chat logs" ON public.ai_chat_logs FOR SELECT TO authenticated USING (public.is_admin());
...
GRANT SELECT, UPDATE, DELETE ON public.broken_image_logs TO authenticated;
...
CREATE POLICY "Staff read broken images" ON public.broken_image_logs FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'::public.app_role) OR public.has_role(auth.uid(), 'moderator'::public.app_role));
...
CREATE POLICY "Staff delete broken images" ON public.broken_image_logs FOR DELETE TO authenticated USING (public.is_admin());
...
CREATE POLICY "Admins can manage backup settings" ON public.backup_settings FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
DROP TRIGGER IF EXISTS set_updated_at ON public.backup_settings;
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.backup_settings FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE POLICY "Admins can manage backup history" ON public.backup_history FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY "Public can read active partnerships" ON public.partnerships FOR SELECT TO anon, authenticated USING (status = 'active' OR public.is_admin());
DROP POLICY IF EXISTS "Admin manage partnerships" ON public.partnerships;
CREATE POLICY "Admin manage partnerships" ON public.partnerships FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
DROP TRIGGER IF EXISTS set_updated_at ON public.partnerships;
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.partnerships FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

CREATE OR REPLACE FUNCTION public.increment_news_view(p_news_id uuid)
RETURNS integer LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_count integer;
BEGIN
  UPDATE public.news SET views_count = COALESCE(views_count, 0) + 1, updated_at = now()
  WHERE id = p_news_id RETURNING views_count INTO v_count;
...
DECLARE v_count integer;
BEGIN
  UPDATE public.news SET shares_count = COALESCE(shares_count, 0) + 1, updated_at = now()
  WHERE id = p_news_id RETURNING shares_count INTO v_count;
  RETURN COALESCE(v_count, 0);
END; $$;
