...
CREATE POLICY "dataroom_signatories_admin_all" ON public.dataroom_signatories FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
DROP TRIGGER IF EXISTS set_updated_at_dataroom_signatories ON public.dataroom_signatories;
CREATE TRIGGER set_updated_at_dataroom_signatories BEFORE UPDATE ON public.dataroom_signatories FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TABLE IF NOT EXISTS public.dataroom_publications (
  ...
  file_url text, video_url text, platform_url text, platform_login text, platform_password text,
  ...
  cover_url text, preview_title text, preview_description text, preview_image_url text, screenshot_url text,
  ...
  source_file_name text, source_file_size bigint, source_mime_type text,
  views_count integer NOT NULL DEFAULT 0, downloads_count integer NOT NULL DEFAULT 0,
  created_by uuid,
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE POLICY "dataroom_publications_admin_all" ON public.dataroom_publications FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
DROP TRIGGER IF EXISTS set_updated_at_dataroom_publications ON public.dataroom_publications;
CREATE TRIGGER set_updated_at_dataroom_publications BEFORE UPDATE ON public.dataroom_publications FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
CREATE OR REPLACE FUNCTION public.dataroom_validate_workflow() RETURNS trigger LANGUAGE plpgsql SET search_path = public AS $$
BEGIN
  IF NEW.workflow_status = 'published' AND NEW.published_at IS NULL THEN NEW.published_at = now(); END IF;
...
CREATE TRIGGER trg_dataroom_validate_workflow BEFORE INSERT OR UPDATE ON public.dataroom_publications FOR EACH ROW EXECUTE FUNCTION public.dataroom_validate_workflow();
...
CREATE POLICY "dataroom_sessions_admin_select" ON public.dataroom_sessions FOR SELECT TO authenticated USING (public.is_admin());
DROP POLICY IF EXISTS "dataroom_sessions_admin_revoke" ON public.dataroom_sessions;
CREATE POLICY "dataroom_sessions_admin_revoke" ON public.dataroom_sessions FOR DELETE TO authenticated USING (public.is_admin());
CREATE INDEX IF NOT EXISTS idx_dataroom_sessions_expires_at ON public.dataroom_sessions(expires_at);
CREATE OR REPLACE FUNCTION public.purge_expired_dataroom_sessions() RETURNS integer LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE removed integer;
BEGIN DELETE FROM public.dataroom_sessions WHERE expires_at < now(); GET DIAGNOSTICS removed = ROW_COUNT; RETURN removed; END; $$;
...
CREATE TABLE IF NOT EXISTS public.dataroom_access_logs (
  ...
  action text NOT NULL, progress_pct integer, ip_address text, user_agent text, device_type text,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE POLICY "dataroom_access_logs_admin_select" ON public.dataroom_access_logs FOR SELECT TO authenticated USING (public.is_admin());
...
CREATE POLICY "dataroom_comments_admin_all" ON public.dataroom_comments FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE TABLE IF NOT EXISTS public.dataroom_intents (
  ...
  intent_type text NOT NULL, message text NOT NULL, status text NOT NULL DEFAULT 'nouveau',
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE POLICY "dataroom_intents_admin_all" ON public.dataroom_intents FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE TABLE IF NOT EXISTS public.dataroom_versions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  publication_id uuid NOT NULL REFERENCES public.dataroom_publications(id) ON DELETE CASCADE,
  version_number integer NOT NULL DEFAULT 1,
  title text, description text, file_url text, source_file_name text, source_file_size bigint, source_mime_type text,
  snapshot jsonb NOT NULL DEFAULT '{}'::jsonb, change_note text, created_by uuid,
  created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE POLICY "dataroom_versions_admin_all" ON public.dataroom_versions FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE TABLE IF NOT EXISTS public.dataroom_review_comments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  publication_id uuid NOT NULL REFERENCES public.dataroom_publications(id) ON DELETE CASCADE,
  author_id uuid, author_name text, body text NOT NULL, status_at_comment text,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE POLICY "dataroom_review_comments_admin_all" ON public.dataroom_review_comments FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE TABLE IF NOT EXISTS public.dataroom_download_links (
  ...
  email text, visibility_scope text NOT NULL DEFAULT 'nda',
  token_hash text NOT NULL UNIQUE,
  expires_at timestamptz NOT NULL DEFAULT (now() + interval '24 hours'),
  max_uses integer NOT NULL DEFAULT 1, used_count integer NOT NULL DEFAULT 0,
  revoked boolean NOT NULL DEFAULT false, last_used_at timestamptz, created_by uuid,
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE POLICY "dataroom_download_links_admin_all" ON public.dataroom_download_links FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
DROP TRIGGER IF EXISTS set_updated_at_dataroom_download_links ON public.dataroom_download_links;
CREATE TRIGGER set_updated_at_dataroom_download_links BEFORE UPDATE ON public.dataroom_download_links FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
CREATE INDEX IF NOT EXISTS idx_dataroom_download_links_expires_at ON public.dataroom_download_links(expires_at);

CREATE OR REPLACE FUNCTION public.increment_dataroom_view(_publication_id uuid) RETURNS void LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
...
CREATE OR REPLACE FUNCTION public.increment_dataroom_download(_publication_id uuid) RETURNS void LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
